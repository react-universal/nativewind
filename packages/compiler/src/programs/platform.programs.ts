...
export const compileProjectFiles = (platform: string) =>
  Effect.gen(function* () {
    ...
    const { refreshCssOutput } = yield* TwinFileSystem;
    const { createDocumentByPath, compileDocument } = yield* TwinDocumentsContext;
    ...
    return yield* Stream.fromIterableEffect(ctx.state.projectFiles.ref).pipe(
      ...
      Stream.flatMap((file) =>
        Stream.fromIterableEffect(
          compileDocument(file, platform).pipe(Effect.map((jsxElements) => jsxElements)),
        ),
      ),
      Stream.run(
