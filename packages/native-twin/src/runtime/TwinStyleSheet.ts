...
import { TwinSheetHandler } from './SheetHandler';

export type StyleSheetProcessor = (entries: SheetEntry[]) => RuntimeSheetDeclaration[];

const getTwinObjectKey = (data: TwinInjectedObject) => {
...
export abstract class StyleSheetAdapter<Theme extends __Theme__ = __Theme__> {
  ...
  private registry = new Map<string, TwinSheetHandler>();
  ...
  inject(data: TwinInjectedObject[]) {
    for (const component of data) {
      const key = getTwinObjectKey(component);
      ...
      if (!this.registry.has(key)) {
        ...
        this.registry.set(key, new TwinSheetHandler(component));
      }
    }
  }
